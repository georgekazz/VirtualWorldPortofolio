...
<body class="bg-gray-900 text-white min-h-screen flex flex-col">
    ...
    <main class="flex-grow container mx-auto p-6">
        ...
        <!-- Modal -->
        <div id="projectModal"
            class="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 hidden px-4 py-10">
            <div class="bg-gray-800 rounded-xl shadow-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto relative p-6">
                <!-- Κουμπί κλεισίματος -->
                <button id="closeModalBtn"
                    class="absolute top-4 right-4 text-gray-400 hover:text-white text-2xl font-bold">&times;</button>
                ...
                <form action="{{ route('admin.projects.store') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div>
                        <label for="short_description" class="block mb-1 font-medium text-gray-300">Σύντομη
                            Περιγραφή</label>
                    ...
                    <div>
                        <label for="full_description" class="block mb-1 font-medium text-gray-300">Πλήρης
                            Περιγραφή</label>
                    ...
                    <div>
                        <label for="thumbnail" class="block mb-1 font-medium text-gray-300">Thumbnail (εικόνα
                            προφίλ)</label>
                    ...
                    <div>
                        <label for="screenshots" class="block mb-1 font-medium text-gray-300">Screenshots (πολλαπλές
                            εικόνες)</label>
                    ...
                    <div>
                        <label for="links" class="block mb-1 font-medium text-gray-300">Σύνδεσμοι (π.χ. GitHub, Live
                            Site)</label>
                        <div id="linkInputsWrapper" class="space-y-2">
                            <input type="text" name="links[]"
                                class="w-full rounded-md border border-gray-600 bg-gray-900 px-3 py-2 text-white focus:outline-none focus:border-purple-500"
                                placeholder="Π.χ. https://github.com/username/project">
                        </div>
                        <button type="button" id="addLinkBtn"
                            class="mt-2 bg-gray-700 text-white px-4 py-2 rounded-md hover:bg-gray-600 transition">
                            + Προσθήκη Συνδέσμου
                        </button>
                    ...
                    <div class="space-y-4">
                        <div>
                            <label for="education_level" class="block text-sm font-medium text-white">Επίπεδο
                                Εκπαίδευσης</label>
                        ...
                        <div>
                            ...
                            <select name="class_level" id="class_level"
                                class="mt-1 block w-full rounded-md border-gray-300 bg-gray-700 text-white">
                                <option value="">-- Επιλέξτε --</option>
                                <option value="Α'" @if(old('class_level', $project->class_level ?? '') == "Α'") selected
                                @endif>Α'</option>
                                <option value="Β'" @if(old('class_level', $project->class_level ?? '') == "Β'") selected
                                @endif>Β'</option>
                                <option value="Γ'" @if(old('class_level', $project->class_level ?? '') == "Γ'") selected
                                @endif>Γ'</option>
                                <option value="Δ'" @if(old('class_level', $project->class_level ?? '') == "Δ'") selected
                                @endif>Δ'</option>
                                <option value="Ε'" @if(old('class_level', $project->class_level ?? '') == "Ε'") selected
                                @endif>Ε'</option>
                                <option value="ΣΤ'" @if(old('class_level', $project->class_level ?? '') == "ΣΤ'") selected
                                @endif>ΣΤ'</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="year" id="year"
                                ...
                                @for($y = 2012; $y <= date('Y'); $y++)
                                    <option value="{{ $y }}" @if(old('year', $project->year ?? '') == $y) selected @endif>
                                        {{ $y }}
                                    </option>
                                @endfor
                            </select>
                        ...
                        <div>
                            ...
                            <select name="project_type" id="project_type"
                                ...
                                <option value="Άλλο" @if(old('project_type', $project->project_type ?? '') == "Άλλο")
                                selected @endif>Άλλο</option>
                            </select>
                        </div>
                    </div>
        ...
        <script>
            ...
            const linkInputsWrapper = document.getElementById('linkInputsWrapper');

            addLinkBtn.addEventListener('click', () => {
                const newInput = document.createElement('input');
                newInput.type = 'text';
                newInput.name = 'links[]';
                newInput.placeholder = 'Π.χ. https://github.com/username/project';
                newInput.className = 'w-full rounded-md border border-gray-600 bg-gray-900 px-3 py-2 text-white focus:outline-none focus:border-purple-500';
                linkInputsWrapper.appendChild(newInput);
            });
        </script>
            ...
            <div class="overflow-x-auto rounded-lg shadow-lg bg-gray-800">
                <table class="min-w-full divide-y divide-gray-700">
                    <thead class="bg-gray-900">
                        <tr>
                            <th
                                class="px-6 py-3 text-left text-xs font-medium text-purple-400 uppercase tracking-wider">
                                ID</th>
                            <th
                                class="px-6 py-3 text-left text-xs font-medium text-purple-400 uppercase tracking-wider">
                                Τίτλος</th>
                            <th
                                class="px-6 py-3 text-left text-xs font-medium text-purple-400 uppercase tracking-wider">
                                Σύντομη Περιγραφή</th>
                            <th
                                class="px-6 py-3 text-left text-xs font-medium text-purple-400 uppercase tracking-wider">
                                Ημ. Δημιουργίας</th>
                            <th
                                class="px-6 py-3 text-center text-xs font-medium text-purple-400 uppercase tracking-wider">
                                Ενέργειες</th>
                        </tr>
                    </thead>
                    <!-- csrf token -->
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center space-x-3">
                                    <a href="{{ route('admin.projects.edit', $project) }}"
                                        class="text-blue-500 hover:text-blue-700" title="Επεξεργασία"><i
                                            class="fas fa-edit"></i></a>

                                    <form action="{{ route('admin.projects.destroy', $project->id) }}" method="POST"
                                        class="inline"
                                        onsubmit="return confirm('Είσαι σίγουρος ότι θέλεις να διαγράψεις αυτό το project;');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" title="Διαγραφή"
                                            class="text-red-500 hover:text-red-700 bg-transparent border-0 cursor-pointer">
